...
import { useDisputeEscrowForm } from "../../../hooks/single-release/dispute-escrow-form.hook";
import { ResponseDisplay } from "@/components/utils/response-display";

export function DisputeEscrowForm() {
  const { form, loading, response, onSubmit } = useDisputeEscrowForm();
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={loading}>
            {loading ? "Disputing Escrow..." : "Dispute Escrow"}
          </Button>
        </form>
      </Form>
